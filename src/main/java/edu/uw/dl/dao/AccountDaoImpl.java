...
public class AccountDaoImpl implements AccountDao {
    ...
    private static final String ADDRESS_FILE = "Address.properties";
    /* The name of the file holding the crdit card data */

    private static final String CREDITCARD_FILE = "Creditcard.txt";
    ...
    public Account getAccount(String accountName) {
        ...
        if (accountDir.exists() && accountDir.isDirectory()) {
            try {
                File inFile = new File(accountDir, ACCOUNT_FILE);
                in = new FileInputStream(inFile);
                account = AccountSer.read(in);
                in.close();

                inFile = new File(accountDir, ADDRESS_FILE);
                if (inFile.exists()) {
                    in = new FileInputStream(inFile);
                ...
                inFile = new File(accountDir, CREDITCARD_FILE);
                if (inFile.exists()) {
                    in = new FileInputStream(inFile);
    ...
    public synchronized void setAccount(Account account) throws AccountException {
        ...
        try {
            ...
            if (address != null) {
                outFile = new File(accountDir, ADDRESS_FILE);
                out = new FileOutputStream(outFile);
            ...
            if (card != null) {
                outFile = new File(accountDir, CREDITCARD_FILE);
                out = new FileOutputStream(outFile);
     ...
     */
    private void deleteFile(final File accountsDir) {
